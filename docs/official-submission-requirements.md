...
| Working live URL accessible in ChatGPT's in-app browser or Chrome 149+ with WebMCP enabled | **PASS** | [Production](https://stillhere-azure.vercel.app) is public, requires no login, and was exercised in the in-app browser. | Recheck after the final video-link commit. |
| Project runs consistently and matches its description/video | **PASS for app; video pending** | Production flow, offline shell, responsive states, and deterministic tests are recorded in [`final-verification.md`](final-verification.md). | Record a video that shows only demonstrated behavior. |
...
| Detectable open-source license | **PASS** | Root [`LICENSE`](../LICENSE) is MIT and GitHub currently detects it as MIT. | Recheck the repository About area before submission. |
...
| Text explains WebMCP fit, better UX, new human-agent collaboration, and implementation | **PASS — prepared** | Copy-ready answers are in [`devpost-submission.md`](devpost-submission.md). | Review and paste them into Devpost. |
| Public YouTube demo, with audio, clearly showing the working project and WebMCP | **NEEDS HUMAN ACTION** | Recording package is in [`video-final.md`](video-final.md). No video URL has been fabricated. | Record, upload with visibility set to **Public**, and supply the URL. |
| Video is strictly under three minutes | **NEEDS HUMAN ACTION** | Script targets about 2:45. | Confirm the exported duration is below 3:00. |
| Video avoids unauthorized trademarks, music, and copyrighted material | **NEEDS HUMAN ACTION** | The recording plan uses only StillHere UI, narration, and fictional data. | Record without copyrighted music or unrelated branded content. |
| English submission materials, or English translations | **PASS — prepared** | Repository and submission package are in English. | Keep the final video narration/captions in English. |
| Project is original, entrant-owned, and third-party materials are authorized | **NEEDS ENTRANT CONFIRMATION** | Repository assets are the StillHere UI/screenshots; dependency and obvious asset review found no copied proprietary product assets. | Entrant must make the legal ownership/authorization representation. |
...
| Register on Devpost and complete every required submission-form field before the deadline | **NEEDS HUMAN ACTION** | Copy-ready project text, links, screenshots, and testing instructions are prepared. | Join/confirm registration, complete form-only fields, upload assets, and submit before the deadline. |
| Keep the project free and available without testing restrictions through judging | **PASS now / FREEZE PENDING** | No account or credentials are required. | Keep the submitted deployment online and freeze it after submission. |
